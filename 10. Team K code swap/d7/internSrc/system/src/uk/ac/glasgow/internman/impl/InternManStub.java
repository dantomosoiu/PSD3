...
public class InternManStub implements InternMan {
	...
	public Advertisement selectAdvertisement(Integer index) {
		return adds.getAdvert(index);
	}

	@Override
	public Role selectRole(Integer advertisementIndex, Integer roleIndex) {
		return adds.getAdvert(advertisementIndex).getRoles().get(roleIndex);
	}
	...
	public void notifyAcceptedOffer(Role role, String managerName,
			String managerEmail) {
		if (currentUser instanceof Student){
			System.out.println("Student send e-mail to"+ managerName + " - "+ managerEmail + " about "+ role.getTitle());
		}
	...
	public void approveAcceptedOffer(String matriculation) {
		if(currentUser instanceof Visitor){
			users.getStudents().get(matriculation).getInternship().setStatus(InternshipStatus.APPROVED);
		}
	}
	...
	public void assignAcademicVisitor(String matriculation, String visitorName) {
		if(currentUser instanceof Visitor){
			users.getStudents().get(matriculation).getInternship().getVisit().setVisitor((VisitorImpl) currentUser);
		}
	...
	public void recordVisitAssessment(String matriculation, UoGGrade grade,
		...
		if(currentUser instanceof Visitor){
			...
			Visit v = new VisitImpl((VisitorImpl) currentUser, description, grade);
			users.getStudents().get(matriculation).getInternship().setVisit(v);
			System.out.println("Email send to student: "+users.getStudents().get(matriculation).getEmail());
			System.out.println("Email send to employee: "+users.getStudents().get(matriculation).getInternship().getEmployer().getEmailAddress());
		}
	}
